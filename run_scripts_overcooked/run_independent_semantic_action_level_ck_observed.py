...
from agents.IndependentActiveInferenceWithDynamicPoliciesCkObserved import utils as dyn_utils
from generative_models.MA_ActiveInference_Monotonic.Overcooked.cramped_room.IndependentWithSemanticPoliciesActionLevelCkObserved.model_init import (
...
def run_agent_vs_env_scenarios():
    ...
    try:
        from agents.IndependentActiveInferenceWithDynamicPoliciesCkObserved.agent import Agent
        from generative_models.MA_ActiveInference_Monotonic.Overcooked.cramped_room.IndependentWithSemanticPoliciesActionLevelCkObserved import (
            A_fn,
            B_fn,
            C_fn,
            D_fn,
    ...
    def run_one_episode(episode_name, seed=None):
        ...
        agent_0.reset()
        agent_1.reset()
        # v2 (CkObserved) only: reset the ground-truth checkbox tracker at
        # episode start so ck_put1/2/3/ck_plated observations don't leak
        # state across episodes/seeds within the same process.
        env_utils.reset_checkbox_ground_truth()
