"""
FullyCollective (Optimized + CollisionFix stack), semantic-action-level, cramped_room.

FullyCollective is IndividuallyCollective's own decision rule (build the
20x20=400 joint semantic-pair policy space, evaluate joint EFE from one set
of beliefs, pick the single best pair) run by exactly ONE thinking agent
instead of two. Agent_0 is the "brain": it perceives, holds beliefs, and runs
the full IC-style infer_policies() once per step. Agent_1 is a pure follower:
it has no beliefs and never runs inference -- it only supplies its own
real, ground-truth-BFS'd semantic path menu (pure geometry, not belief), and
then blindly executes whichever primitive the brain's single winning joint
policy assigns to it. Both halves of every executed action are decoded from
the one joint policy index the brain selects (ego half via integer division,
follower half via the remainder) -- there is no second, independently-run
policy inference anywhere in this script.

This is a fully independent copy of IndividuallyCollective's Optimized +
CollisionFix run script -- see ai/02-debug.md sections J.8-J.9 for what the
CollisionFix generative model and PROGRESS_SUCCESS_PROB fix actually do. Kept
isolated from IC (separate agent/generative-model packages, not shared
imports) so changes to one paradigm can never affect the other.
"""
...
from agents.FullyCollectiveActiveInferenceWithDynamicPoliciesOptimizedCollisionFix import utils as dyn_utils
from generative_models.MA_ActiveInference_Monotonic.Overcooked.cramped_room.FullyCollectiveWithSemanticPoliciesActionLevelCollisionFix.model_init import (
...
def _sample_or_argmax_policy_index(agent) -> int:
    ...
    single shared global stream via sequential `np.random.seed()` calls.
    """
...
def run_agent_vs_env_scenarios():
    ...
    try:
        from agents.FullyCollectiveFixedPoliciesOptimizedCollisionFix.agent import Agent
        from generative_models.MA_ActiveInference_Monotonic.Overcooked.cramped_room.FullyCollectiveWithSemanticPoliciesActionLevelCollisionFix import (
            A_fn,
            B_fn,
            C_fn,
            D_fn,
    ...
    def run_one_episode(episode_name, seed=None):
        ...
        for step in range(1, max_steps_per_scenario + 1):
            ...
            # 1) real-time state inference: ONLY the brain (agent_0) perceives and
            #    updates beliefs. The follower (agent_1) has no brain -- it never
            #    calls infer_states, never holds a belief, never runs EFE. This is
            #    the one substantive difference from IC: FC has a single thinking
            #    agent, not two.
            agent_0.infer_states(obs_0)

            # 2) build policy-generation state from current env state. policy_state_1
            #    is still needed here -- not for any belief update, but purely to
            #    compile the follower's real, ground-truth-BFS'd semantic path menu
            #    (pure geometry, exactly the same "borrow the partner's real path
            #    list" step IC's own brain already does -- see ai/02-debug.md section
            #    J's note on why path-compilation is deliberately belief-independent).
            ...
            # 3) compile semantic→primitive paths for both agents. agent_1.update_policies
            #    only does ground-truth BFS pathfinding (no belief dependency), so it's
            #    safe to call even though agent_1 never runs infer_states.
            ...
            ego0_paths = list(agent_0.policies)   # brain's own compiled paths
            ego1_paths = list(agent_1.policies)   # follower's real compiled paths (geometry only)
            ...
            # 4) Fully Collective: ONE brain builds the N×N joint primitive policy
            #    space -- ego=agent_0 (itself), partner=agent_1 (the follower) --
            #    and runs the exact same joint-EFE evaluation IC's own brain runs.
            #    There is no second, independently-computed joint policy set: FC has
            #    exactly one 400-pair decision per step, not two.
            ...
            # 5) select the single best joint policy. Policy index k = ego_sem *
            #    N_SEMANTIC + partner_sem (see _build_joint_primitive_policies), so
            #    BOTH halves of the executed action come from decoding this one
            #    winning index -- the brain's own semantic choice via integer
            #    division, and the primitive it assigns to the follower via the
            #    remainder. The follower never makes its own decision; it is simply
            #    told the second half of the one joint policy the brain picked.
            pol_idx_0 = _sample_or_argmax_policy_index(agent_0)

            ego0_sem = int(pol_idx_0) // N_SEMANTIC_ACTIONS
            ego1_sem = int(pol_idx_0) % N_SEMANTIC_ACTIONS
            ...
            # Only the brain carries beliefs forward -- the follower has none to update.
            ...
            if verbose:
                ...
                print(_belief_table(np, qs_0, model_init_agent, title="Beliefs (brain, agent_0 -- agent_1 has no beliefs)"))

                print("    Policy beliefs (single brain decision, agent_0):")
                ...
                print("    Joint policy idx={}  {}  (both halves decoded from this one index)".format(
                    int(pol_idx_0), _fmt_joint_semantic(pol_idx_0, N_SEMANTIC_ACTIONS)
                ))
    ...
    run_one_episode(
        "FullyCollective (one IC-brain + one puppet follower): cramped_room (seed=76)",
        seed=76,
    )
