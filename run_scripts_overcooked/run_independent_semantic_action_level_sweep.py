        ...
        for step in range(1, max_steps_per_scenario + 1):
            ...
            if want_detail and map_before is not None:
                ...
                sal_detail.write_ind_step(
                    ...
                    # --log-jsonl always forced the full ~20-policy q_pi array into
                    # every JSONL record regardless of --log-full-q-pi, defeating a
                    # top-k-only logging request. Now driven by --log-full-q-pi alone;
                    # top_policies (--policy-log-top-k) is always included separately.
