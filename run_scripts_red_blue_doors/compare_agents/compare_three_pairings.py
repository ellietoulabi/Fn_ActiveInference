"""
Compare the three two-agent AIF pairings on RedBlueButton under the same settings.

Runs Fully Collective, Individually Collective, and Independent paradigms with
identical seeds, episodes, episodes-per-config, and max-steps, then prints a
comparison table and optionally saves CSV/plots.

Fair comparison: all three scripts use policy_len=2, gamma=2.0, alpha=1.0, num_iter=16,
same env (TwoAgentRedBlueButton 3x3), and same config generation.
"""
...
def run_paradigm(script_path: Path, seeds: int, episodes: int, episodes_per_config: int,
                 max_steps: int, stats_output: Path, quiet: bool = True,
                 episode_progress: bool = False) -> dict:
    ...
    cmd = [
        ...
        "--episodes-per-config", str(episodes_per_config),
...
def main():
    parser = argparse.ArgumentParser(
        description="Compare Fully Collective, Individually Collective, and Independent two-agent AIF pairings."
    )
    ...
    parser.add_argument("--episodes-per-config", type=int, default=40, help="Episodes per map config (default: 40)")
    parser.add_argument("--max-steps", type=int, default=50, help="Max steps per episode (default: 50)")
    parser.add_argument("--output-dir", type=str, default=None,
                        help="Directory for stats JSONs and comparison CSV (default: logs/compare_three_pairings_<timestamp>)")
    ...
    parser.add_argument("--episode-progress", action="store_true",
                        help="Pass --episode-progress to each paradigm (show per-step progress within each episode)")
    ...
    if args.output_dir:
        ...
        output_dir = project_root / "logs" / f"compare_three_pairings_{timestamp}"
    ...
    if not args.no_run:
        print("=" * 80)
        print("COMPARING THREE TWO-AGENT AIF PAIRINGS (same seeds, episodes, env settings)")
        print("=" * 80)
        print(f"  Seeds: {args.seeds}  Episodes per seed: {args.episodes}  Episodes per config: {args.episodes_per_config}  Max steps: {args.max_steps}")
        print()
    ...
    print("=" * 80)
    print(f"{'Paradigm':<25} {'Success %':>10} {'Mean reward':>12} {'Mean steps':>12} {'Total episodes':>15}")
    print("-" * 80)
    for name in SCRIPTS:
        s = all_stats[name]
        sr = s["success_rate"]
    ...
    with open(comparison_path, "w") as f:
        f.write("paradigm,success_rate,mean_reward,std_reward,mean_steps,std_steps,total_episodes,total_successes\n")
        for name in SCRIPTS:
            s = all_stats[name]
            f.write(f"{name},{s['success_rate']},{s['mean_reward']},{s['std_reward']},{s['mean_steps']},{s['std_steps']},{s['total_episodes']},{s['total_successes']}\n")
