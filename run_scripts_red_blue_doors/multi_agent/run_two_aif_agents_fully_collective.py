...
def run_seed_experiment(seed, num_episodes, episodes_per_config, max_steps,
    ...
    for episode in tqdm(episode_iter, disable=verbose, desc=f"Seed {seed}", leave=True, unit="ep", position=1):
        ...
        if (episode - 1) % episodes_per_config == 0 or env is None:
            ...
            # Construct the agent ONCE per seed, not on every config change -- see the
...
def main():
    ...
    csv_filename = f"two_aif_agents_fully_collective_{SEED_TAG}_ep{NUM_EPISODES_PER_SEED}_step{MAX_STEPS}_redblue_{timestamp}.csv"
    ...
    if args.log_policy_beliefs:
        jsonl_filename = f"two_aif_agents_fully_collective_{SEED_TAG}_ep{NUM_EPISODES_PER_SEED}_step{MAX_STEPS}_redblue_{timestamp}_policy_log.jsonl"
        ...
        try:
            for seed_idx, seed in enumerate(SEEDS_TO_RUN):
                ...
                configs_path = args.configs_file if args.configs_file is not None else (
                    log_dir / f"two_aif_agents_fully_collective_seed{seed}_ep{NUM_EPISODES_PER_SEED}_"
    ...
    stats_filename = f"two_aif_agents_fully_collective_{SEED_TAG}_ep{NUM_EPISODES_PER_SEED}_step{MAX_STEPS}_redblue_{timestamp}_stats.json"
