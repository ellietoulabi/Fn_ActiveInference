...
from generative_models.SA_ActiveInference.RedBlueButtonIndependent import B as B_module
...
def serialize_top_policies_for_json(agent, top_k, action_names=None):
    """Top-k policies for JSONL (SA: policy is list of action indices)."""
...
def _top_policies_bar(agent, action_names, top_k=5):
    """Top-k policies with bar for one agent (SA: each policy is list of actions)."""
    lines = []
    ...
    for (pol, prob, idx) in agent.get_top_policies(top_k=top_k):
        pol_str = "→".join([action_names.get(int(a), str(a))[:2] for a in pol])
        bar = "█" * int(prob * 20) + "░" * (20 - int(prob * 20))
        lines.append(f"   [{pol_str:>8}] {bar} {prob:.3f}")
    return "\n".join(lines)


def print_step_info(step, env_obs, agent1, agent2, action1, action2, reward, cumulative_reward, info, env, action_names):
    """Compact step info: obs, both agents' state beliefs + policy bars, actions, outcome."""
    print(f"\n{'═'*80}")
...
# =============================================================================
# Custom helper functions for converting two-agent env to single-agent model format
# =============================================================================
...
def env_obs_to_log_obs(env_obs, width=3):
    """Build dict of model-style indices for CSV/JSONL logging (agent1_pos, agent2_pos, etc.)."""
...
def two_agent_obs_to_single_agent_obs(env_obs, agent_id, width=3):
    ...
    return {
        ...
        'blue_button_pressed': env_obs['blue_button_pressed'],
        'win_lose_neutral': env_obs['win_lose_neutral'],
        'button_just_pressed': env_obs['button_just_pressed'],
    }
...
def _validate_sa_model(env):
    """
    Sanity checks to catch common SA A/B/C/D interface mismatches early.
    """
    state_factors = list(model_init.states.keys())
    obs_modalities = list(model_init.observations.keys())

    d_config = get_D_config_for_agent(env, agent_id=1, width=env.width)
    D = D_fn(d_config)
    ...
    for f in state_factors:
        ...
        s = float(np.sum(np.array(qs_next[f])))
...
# =============================================================================
# Agent creation and episode running
# =============================================================================

def create_aif_agent(agent_id, env, action_selection="deterministic"):
    ...
    agent = Agent(
        ...
        # Default is "deterministic". The root
    ...
    # Get config from environment for this specific agent
    d_config = get_D_config_for_agent(env, agent_id, width=env.width)
...
def run_episode(
    ...
    for step in step_iter:
        single_agent_obs1 = two_agent_obs_to_single_agent_obs(obs, agent_id=1)
        single_agent_obs2 = two_agent_obs_to_single_agent_obs(obs, agent_id=2)
        model_obs1 = env_utils.env_obs_to_model_obs(single_agent_obs1)
        model_obs2 = env_utils.env_obs_to_model_obs(single_agent_obs2)
        ...
        if verbose:
            print_step_info(
                step, obs, agent1, agent2, action1, action2,
                reward, episode_reward, info, env, action_names,
            )
        elif print_steps:
        ...
        if policy_log_fh is not None:
            log_entry = {
                ...
                "agent1": {"top_policies": serialize_top_policies_for_json(agent1, log_policy_top_k, action_names)},
                "agent2": {"top_policies": serialize_top_policies_for_json(agent2, log_policy_top_k, action_names)},
            }
            if log_full_q_pi:
...
def main():
    ...
    parser.add_argument("--press-success-prob", type=float, default=None, help="Override B.PRESS_SUCCESS_PROB (default 0.85) for a noise-level sweep. Patches the RedBlueButtonIndependent.B module before any agent is built.")
        ...
        try:
            for seed_idx, seed in enumerate(SEEDS_TO_RUN):
                ...
                configs_path = args.configs_file if args.configs_file is not None else (
                    log_dir / f"two_aif_agents_independent_seed{seed}_ep{NUM_EPISODES_PER_SEED}_"
    ...
    plot_path = output_dir / f"two_aif_agents_independent_seeds{NUM_SEEDS}_ep{NUM_EPISODES_PER_SEED}.png"
