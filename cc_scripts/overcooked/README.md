# Compute Canada jobs — Overcooked semantic action-level (ind / ic / fc)
...
> These scripts live in `cc_scripts/overcooked/`. For RedBlueButton jobs, see `cc_scripts/redbluebutton/README.md`.
...
RedBlueButton jobs (`two_aif_*.sh`, `eight.sh`, `nine.sh`, `three_plus_ppo.sh`) live in `cc_scripts/redbluebutton/` and are unrelated to the semantic-action-level stack above.
...
2. Clone the repo into `$SLURM_TMPDIR`, create a venv, `pip install -r cc_scripts/overcooked/requirements-cc-sal.txt`
...
MAX_STEPS=50 sbatch cc_scripts/overcooked/ind_semantic_action_level.sh

# More seeds (edit --array in the script, or override)
sbatch --array=0-9 cc_scripts/overcooked/ind_semantic_action_level.sh   # 10 seeds → episodes 76–85

# Single seed for debugging
sbatch --array=2 cc_scripts/overcooked/fc_semantic_action_level.sh      # only index 2 → ep 78, brain 1002
```

To change `gamma` / `alpha` for a whole batch:

```bash
GAMMA=8.0 ALPHA=16.0 sbatch cc_scripts/overcooked/ic_semantic_action_level.sh
```
...
| `opencv-python` / dummy wheel error during `pip install` | Scripts must use `overcooked/requirements-cc-sal.txt`, not `requirements.txt`. OpenCV is not needed for these runs (terminal logs only). |
