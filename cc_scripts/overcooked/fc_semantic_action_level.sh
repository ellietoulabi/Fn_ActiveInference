...
#SBATCH --job-name=aif_fc_sal_Bnoise05alpha4
#SBATCH --array=0-9                   # 10 seeds (one episode per task)
#SBATCH --cpus-per-task=16
#SBATCH --mem=16G
#SBATCH --time=4-00:00
#SBATCH --output=fc_sal_Bnoise05alpha4_%A_%a.out
...
mkdir -p "$CSV_DIR"
LOG_FILE="$SLURM_TMPDIR/fc_sal__Bnoise05alpha4_ep${EP_SEED}_brain${AGENT_SEED}.log"
