...
def A_checkbox_obs(checkbox_value: int, modality: str) -> np.ndarray:
    """
    v2 (CkObserved) only: direct observation of a recipe-progress checkbox
    (ck_put1/2/3, ck_plated). Same noise level as the other directly-observed
    factors (self_pos, self_held, pot_state) -- deliberately not the
    noise-free pattern used for counters, so this stays self-correcting via
    the same floor/Bayesian-combine mechanics as everything else IND directly
    observes, rather than producing an exact one-hot likelihood.
    """
    n = len(model_init.observations[modality])
    return _noisy_categorical(int(checkbox_value), n, A_NOISE_LEVEL)
...
def A_fn(state_indices: dict) -> dict[str, np.ndarray]:
    ...
    obs["ck_put1_obs"] = A_checkbox_obs(state_indices.get("ck_put1", 0), "ck_put1_obs")
    obs["ck_put2_obs"] = A_checkbox_obs(state_indices.get("ck_put2", 0), "ck_put2_obs")
    obs["ck_put3_obs"] = A_checkbox_obs(state_indices.get("ck_put3", 0), "ck_put3_obs")
    obs["ck_plated_obs"] = A_checkbox_obs(state_indices.get("ck_plated", 0), "ck_plated_obs")
