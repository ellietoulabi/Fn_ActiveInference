...
def B_checkboxes(
    ...
    """
    v3 (CkRemoved): only ck_delivered remains. ck_put1/2/3 and ck_plated were
    deleted -- confirmed dead computation in ai/02-debug.md section I.1
    (never observed, never eligible for info gain, never read by anything
    else in the model, including each other or ck_delivered -- ck_delivered's
    own driving signal below was already computed directly from self_held/
    position, never from the other checkboxes, so removing them changes
    nothing about this function's output). `q_pot_next` kept as a parameter
    for call-site compatibility with v1/v2, but is unused here.
...
def B_fn_primitive_step(
    ...
    **kwargs,
    ...
    for factor, deps in state_state_dependencies.items():
        if factor in ("ck_delivered",):
            if factor == "ck_delivered":
