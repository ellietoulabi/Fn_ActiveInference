"""
FullyCollective paradigm model init for Overcooked - Cramped Room layout (Monotonic).

This is the *FullyCollective* variant of the semantic-action-level model.

Structural identity with IndividuallyCollective:
    The hidden state, observation modalities, and transition dependencies are
    exactly the same as in
    `IndividuallyCollectiveWithSemanticPoliciesActionLevel`.  One agent reasons
    about both agents' (self / other) positions, orientations, held objects,
    the pot, the modeled counters and the checkbox factors.

Difference from IndividuallyCollective:
    Only ONE of the two physical agents actually runs the active-inference
    machinery.  That agent ("IC brain") plans over joint semantic pairs
    (JOINT_PAIR_LABEL, a_self, a_other) and dictates the action for BOTH
    agents.  The second agent ("puppet") does not maintain its own beliefs
    and does not run policy inference; it simply executes whatever the IC
    brain prescribes for the partner.

This file therefore mirrors IC's model_init verbatim — the difference shows
up only at the runner level (one Agent instance + a puppet executor).
...
# FullyCollective plans over the full joint semantic pair space (self × other).
N_JOINT_PAIR_POLICIES = N_ACTIONS * N_ACTIONS  # 20 × 20 = 400 joint semantic pairs
...
def construct_joint_pair_policies(policy_len: int = 1) -> list[list[tuple]]:
    """
    Enumerate ALL joint semantic-pair policies for the FullyCollective brain.

    Each policy step is the canonical joint-pair tuple:
        (JOINT_PAIR_LABEL, a_self_semantic, a_other_semantic)

    Returns:
        list of policies; each policy is a sequence of `policy_len` joint-pair
        steps. With the default policy_len=1 the result has length
        N_ACTIONS * N_ACTIONS (= 400 for cramped_room).
    """
    from itertools import product
    ...
    one_step_pairs = [
        (JOINT_PAIR_LABEL, int(a_self), int(a_other))
        for a_self in range(N_ACTIONS)
        for a_other in range(N_ACTIONS)
    ]

    if policy_len == 1:
        return [[p] for p in one_step_pairs]

    return [list(p) for p in product(one_step_pairs, repeat=int(policy_len))]
...
#   (PRIMITIVE_POLICY_STEP, primitive_action)        — self-only
#   (PRIMITIVE_POLICY_STEP, a_self, a_other)         — joint primitive timestep
