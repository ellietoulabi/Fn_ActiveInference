...
Identical to the IC env_utils: this module converts an Overcooked
OvercookedState into the IC-style observation dict (self / other / pot /
counters / delivered).  In FullyCollective the single IC brain consumes
observations from its own ego perspective (agent_idx = 0 by default), while
the puppet's observations are only used to compile primitive action plans
for it.
