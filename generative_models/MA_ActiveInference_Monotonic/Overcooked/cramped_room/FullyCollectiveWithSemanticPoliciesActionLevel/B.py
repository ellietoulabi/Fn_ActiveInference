# B.py
"""
Transition p(s' | s, a) model (B) for FullyCollective paradigm — Cramped Room.

Structurally identical to the IC transition model: the single IC brain rolls
state beliefs forward over self / other / pot / counters / checkbox factors.
The FullyCollective paradigm uses the JOINT_PAIR_LABEL policy step
(`(JOINT_PAIR_LABEL, a_self, a_other)`) so the brain plans joint semantic
pairs that prescribe both agents' actions in lockstep.

Policy semantics:
- one policy step may be:
    1) a joint semantic pair: (JOINT_PAIR_LABEL, a0, a1)        — FC default
    2) an interleaved semantic step: (actor, semantic_action)    — back-compat
    3) a scalar interleaved encoding over semantic actions       — back-compat
...
Notes:
- For the FC paradigm both agents always have a semantic plan, so the
  "None / no-op" branch is not normally used.  It is preserved for symmetry
  with IC.
- For primitive timestep execution (when the runner compiles a chosen joint
  pair to primitive moves) the policy step is encoded as
  (PRIMITIVE_POLICY_STEP, a_self_primitive, a_other_primitive) and rolled
  through `B_fn_primitive_step`.
"""
