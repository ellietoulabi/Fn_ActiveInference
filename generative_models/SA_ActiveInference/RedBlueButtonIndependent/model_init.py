...
states = {
    ...
    # Independent's partner is folded into the environment, not modeled as an
    # agent with its own intentions/actions -- but it IS directly observable,
    # so the model tracks its position the same way it tracks a wall or a
    # button: an environmental fact, not a co-planned entity. See
...
observations = {
    "agent_pos": list(range(S)),  # 0..S-1 positions
    "other_pos": list(range(S)),  # 0..S-1 positions (the other agent's position)
...
state_state_dependencies = {
    # agent_pos now also depends on other_pos: the deterministic move target is
    # blocked if it (believed to) coincide with the other agent's current
    # position -- treated as a static obstacle, no modeling of the other
    # agent's own intended move (Independent doesn't co-plan).
    "agent_pos": ["agent_pos", "other_pos"],
    # other_pos has no B-level transition model of its own (Independent
    # doesn't predict how the partner moves) -- treated as static-with-noise,
    # same pattern as button positions, so belief uncertainty grows slightly
    # every planning step until corrected by the next direct observation.
    "other_pos": ["other_pos"],
