...
def A_other_pos(other_pos_idx, num_obs, noise_level=A_NOISE_LEVEL):
    """
    Observation likelihood for the other agent's position -- same noise model
    as our own position (direct, near-noiseless observation).

    Dependencies: other_pos

    Parameters
    ----------
    other_pos_idx : int
        Specific other-agent position state index (0 to S-1)
    num_obs : int
        Number of possible observations (same as S)
    noise_level : float
        Observation noise level
    ...
    -------
    p_obs : array, shape (num_obs,)
        p(observe position o | other agent actually at other_pos_idx)
    """
    pos_obs = np.full(num_obs, noise_level / (num_obs - 1))
    pos_obs[other_pos_idx] = 1.0 - noise_level
    return pos_obs
...
def A_fn(state_indices, prev_state_indices=None, modalities=None):
    ...
    for modality in modalities_to_compute:
        ...
        if modality in ("agent_pos", "other_pos"):
...
def get_observation_likelihood(modality, state_indices):
    ...
    if modality in ("agent_pos", "other_pos"):
        num_obs = len(model_init.states[modality])
...
def predict_obs_from_beliefs(modality, state_beliefs, prev_state_beliefs=None):
    ...
    # Special handling for agent_pos / other_pos (have a noise parameter)
    if modality in ("agent_pos", "other_pos"):
        S = factor_sizes[modality]
        for pos in range(S):
            if state_beliefs[modality][pos] > 1e-10:
                p_obs = A_func(pos, num_obs)
                q_obs += state_beliefs[modality][pos] * p_obs
