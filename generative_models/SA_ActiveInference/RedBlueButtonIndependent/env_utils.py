...
def env_obs_to_model_obs(env_obs, width=3):
    ...
    # other_pos is optional so this function stays backward compatible with
    # any caller not passing it.
    if 'other_position' in env_obs and env_obs['other_position'] is not None:
        ox, oy = env_obs['other_position']
        model_obs['other_pos'] = xy_to_index(int(ox), int(oy), width)
