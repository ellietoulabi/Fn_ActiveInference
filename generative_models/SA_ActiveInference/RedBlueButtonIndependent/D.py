...
def build_D_uncertain(agent_start_pos=0,
    ...
    other_start_pos : int or None
        The other agent's real start position, known/observable at episode
        start the same way our own start position is (not modeled with any
        agency/intention -- see model_init.py's other_pos comment). If None,
        falls back to uniform (matches the old, other_pos-unaware behavior
    ...
    # Other agent's position: certain at its real start position, same as our
    # own -- it's directly observable, just not co-planned.
    if other_start_pos is None:
        D["other_pos"] = np.ones(S) / S
    else:
