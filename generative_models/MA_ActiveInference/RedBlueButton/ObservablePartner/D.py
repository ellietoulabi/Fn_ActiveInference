"""
Prior beliefs (D) for ObservablePartner variant.
"""
...
def build_D(my_start_pos=0, other_start_pos=8, button_pos_uncertainty=True, button_state_uncertainty=False):
    S = model_init.S
    D = {}

    D["my_pos"] = np.zeros(S)
    D["my_pos"][int(my_start_pos)] = 1.0
    ...
    if button_pos_uncertainty:
        ...
        for idx in (int(my_start_pos), int(other_start_pos)):
            if 0 <= idx < S:
                base[idx] = 0.0
        if float(base.sum()) <= 0:
            base = np.ones(S, dtype=float)
