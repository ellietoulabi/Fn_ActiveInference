...
def _compute_new_pos(pos_idx: int, action: int) -> int:
    ...
    if new_idx in model_init.WALL_INDICES:
        return pos_idx
...
def _valid_neighbors(idx: int) -> list[int]:
    """
    Valid neighbor cells for other-agent dynamics: N,S,E,W that are not walls.
    """
    ...
    for nx, ny in candidates:
        if 0 <= nx < model_init.GRID_WIDTH and 0 <= ny < model_init.GRID_HEIGHT:
            ...
            if nidx not in model_init.WALL_INDICES:
                out.append(nidx)
    return out
...
def B_agent_held(qs: dict, action: int) -> np.ndarray:
    ...
    INTERACT rules:
      front ONION + held NONE -> held ONION
      front DISH  + held NONE -> held DISH
      front POT   + held ONION + pot in {POT_0,POT_1,POT_2} -> held NONE (onion deposited)
      front POT   + held DISH  + pot POT_READY -> held SOUP
      front SERVE + held SOUP -> held NONE
    """
    q_pos = np.array(qs["agent_pos"], dtype=float)
    q_ori = np.array(qs["agent_orientation"], dtype=float)
    q_other = np.array(qs["other_agent_pos"], dtype=float)
                    ...
                    for pot_state in range(model_init.N_POT_STATES):
                        ...
                        if action == model_init.INTERACT:
                            if front == model_init.FRONT_BLOCKED_BY_OTHER:
                                ...
                                # deliver (delivery is an OBS event handled outside B)
                                new_held = model_init.HELD_NONE
...
# Main B
# ----------------------------
def B_fn(qs: dict, action: int, collision_awareness: float = 0.5) -> dict[str, np.ndarray]:
    """
