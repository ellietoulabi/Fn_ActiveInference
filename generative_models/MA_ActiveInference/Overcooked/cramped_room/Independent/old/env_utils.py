...
def get_D_config_from_state(state, agent_idx):
    ...
    You can use this to configure your D prior if you want D to be exactly correct
    even if start positions/orientations vary.

    Returns:
        dict with:
          - agent_id (1 or 2) based on which start position matches,
            OR a direct start index if you prefer to modify D_fn accordingly.
    """
    ...
    # If you kept D_fn(agent_id=1/2) with fixed starts (6 and 13),
    # you can infer agent_id from the observed start location.
    # Otherwise you may want to extend D_fn to accept explicit start indices.
    agent_id = 1
    if this_pos_idx == 13:
        agent_id = 2
    ...
    return {
        ...
        "other_pos_noise": 0.0,
        "ori_uniform": False,
        "fixed_orientation": this_ori_idx,
        # If you later extend D_fn to accept explicit indices, you can pass:
