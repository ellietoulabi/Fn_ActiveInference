...
def print_step_info(step, env_obs, agent1, agent2, action1, action2, reward, info, env, action_names):
    ...
    # Agent 1 policies (top 15 for readability)
    ...
    for i in range(0, top_k1, 3):
        ...
        for j in range(3):
            if i + j < top_k1:
                ...
                prob = q_pi1[idx]
                policy_str = '→'.join([action_names.get(a, str(a))[:2] for a in policy])
    ...
    # Agent 2 policies (top 15 for readability)
...
def run_episode(
    ...
    for step in step_iter:
        # Convert observations to single-agent model format for each agent
        ...
        if verbose:
            print_step_info(step, obs, agent1, agent2, action1, action2, reward, info, env, action_names)
        
        # Log to CSV
        if csv_writer is not None:
            # Get beliefs and policies from agents (after state inference and policy evaluation)
    ...
    for episode in tqdm(episode_iter, disable=verbose, desc=f"Seed {seed}", leave=False):
        ...
        if (episode - 1) % episodes_per_config == 0 or env is None:
            env = TwoAgentRedBlueButtonEnv(
                ...
                # env.step_count increments once per JOINT step; align with run_episode max_steps
                max_steps=max_steps,
            )
...
def main():
    ...
    csv_filename = f"two_aif_agents_seeds{NUM_SEEDS_FOR_FILENAME}_ep{NUM_EPISODES_PER_SEED}_{timestamp}.csv"
