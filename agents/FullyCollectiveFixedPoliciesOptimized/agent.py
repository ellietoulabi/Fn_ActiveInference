...
This is FullyCollective's own copy of agents/ActiveInferenceFixedPoliciesOptimized
(IndividuallyCollective's wrapper), rsync'd verbatim and repointed to import from
agents.FullyCollectiveActiveInferenceWithDynamicPoliciesOptimized instead of
agents.IndividuallyCollectiveActiveInferenceWithDynamicPoliciesOptimized. FC's
single "brain" agent runs the exact same decision logic IC's agent does (joint
EFE over the 400-pair semantic space); the only difference between the two
paradigms is execution -- FC has one brain compute the joint decision and force
both agents' primitive actions, IC has two brains independently compute it and
each execute only their own half. Kept as a fully separate package (not a
config flag or an import from IC) so IC and FC can be run, modified, or
...
from agents.FullyCollectiveActiveInferenceWithDynamicPoliciesOptimized.agent import Agent as _BaseAgent
from agents.FullyCollectiveActiveInferenceWithDynamicPoliciesOptimized import control, inference
