...
# =============================================================================
#
# The original get_expected_obs_from_beliefs / get_expected_obs_and_info_gain_unified
...
# positions and button states are all fairly confident), dep_list becomes empty,
...
# residual uncertainty remains (the same "zero-info-gain pathology" documented
# for Overcooked in ai/02-debug.md, section "Deeper issue: flat EFE").
#
...
# same group iff some active modality depends on both. Unlike Overcooked, this
# codebase's RedBlueButton modalities are NOT a clean 1-factor-per-modality
# bijection (e.g. "agent1_on_red_button" depends on both agent1_pos and
# red_button_pos), so the exact decomposition here works at the group level
# rather than the fully-independent single-factor level Overcooked's Exact
# stack achieves. Within a group, states are enumerated exactly (every combo,
# no threshold-based dropping). Because the mean-field belief factorizes
# q(s) = prod_f q(s_f), and no modality spans two different groups, the joint
# predictive entropy is the exact SUM of each group's own joint entropy -- this
# is provably exact, not an approximation, for any dependency structure (a
# strict generalization of the single-factor case).
