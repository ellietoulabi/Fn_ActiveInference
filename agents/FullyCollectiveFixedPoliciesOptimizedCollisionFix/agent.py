...
This is FullyCollective's own copy of agents/ActiveInferenceFixedPoliciesOptimizedCollisionFix
(IndividuallyCollective's wrapper), differing only in which generative model
it's paired with by the run scripts: this stack's run scripts import from
generative_models...FullyCollectiveWithSemanticPoliciesActionLevelCollisionFix
(FC's own copy of IC's collision-fix generative model), whose B.py replaces
the independent B_self_pos/B_other_pos collision check with a joint,
collision-aware B_positions_joint that matches the real environment's exact
collision rule, and includes the PROGRESS_SUCCESS_PROB fix (see ai/02-debug.md,
sections J.8-J.9). This agent/control code is IC's decision logic, run once by
FC's single brain instead of independently by two agents. Kept as a fully
separate package (not a config flag, not an import from IC) so it can be run,
modified, or compared independently without any risk to IC's stacks or any
other paradigm.
...
from agents.FullyCollectiveActiveInferenceWithDynamicPoliciesOptimizedCollisionFix.agent import Agent as _BaseAgent
from agents.FullyCollectiveActiveInferenceWithDynamicPoliciesOptimizedCollisionFix import control, inference
