"""
Plot aggregated comparison across multiple seeds with confidence intervals.

Loads multi-seed data and plots mean ± confidence interval for each agent.
...
def load_and_process_multiseed_log(log_file):
    ...
    if 'seed' not in df.columns:
        raise ValueError("Log file must contain 'seed' column. Use compare_six_agents.py with multiple seeds.")
...
def plot_aggregated_comparison(aggregated, agent_names, episodes_per_config=None, output_path=None):
    ...
    fig.suptitle(f'Aggregated Comparison Across Multiple Seeds (mean ± 95% CI)', 
    ...
    ax1.set_title('Average Reward Convergence (mean ± 95% CI)', fontsize=16, fontweight='bold', pad=15)
...
def main():
    parser = argparse.ArgumentParser(description='Plot aggregated multi-seed comparison (mean values)')
