...
Loads separate seed files from logs directory and plots mean values with confidence intervals.
Each seed file contains data for 8 agents over 2000 episodes.
"""
...
def load_seed_files(logs_dir):
    """Load all seed files from logs directory and aggregate by seed."""
    logs_path = Path(logs_dir)
    
    # Find all CSV files matching the pattern
    seed_files = sorted(logs_path.glob("eight_agents_comparison_*.csv"))
    
    if not seed_files:
        raise ValueError(f"No seed files found in {logs_dir}")
    
    print(f"Found {len(seed_files)} seed files")
    
    # Load all files and extract seed information
    all_data = {}
    agent_names = None
    seeds = []
    
    for seed_file in seed_files:
        # Extract seed from filename (e.g., seed0, seed1, etc.)
        seed_match = re.search(r'seed(\d+)', seed_file.name)
        if seed_match:
            seed_id = int(seed_match.group(1))
        else:
            # Fallback: use the seed value from the CSV file itself
            df_temp = pd.read_csv(seed_file, nrows=1)
            seed_id = df_temp['seed'].iloc[0] if 'seed' in df_temp.columns else None
            if seed_id is None:
                print(f"Warning: Could not extract seed from {seed_file.name}, skipping")
                continue
        
        print(f"Loading seed {seed_id} from {seed_file.name}")
        df = pd.read_csv(seed_file)
        
        # Get unique agents (should be same across all files)
        ...
        # Calculate episode statistics for each agent in this seed
        ...
        # Process each agent for this seed
        if seed_id not in all_data:
            all_data[seed_id] = {}
        
        for agent_name in agent_names:
            agent_df = df[df['agent'] == agent_name].copy()
            all_data[seed_id][agent_name] = calc_episode_stats(agent_df)
    
    # Reorganize data structure: all_data[agent][seed] instead of all_data[seed][agent]
    reorganized_data = {}
    for agent_name in agent_names:
        reorganized_data[agent_name] = {}
        for seed_id in seeds:
            reorganized_data[agent_name][seed_id] = all_data[seed_id][agent_name]
    
    print(f"\nLoaded data for {len(seeds)} seeds: {sorted(seeds)}")
    
    return reorganized_data, agent_names, sorted(seeds)


def aggregate_across_seeds(all_data, agent_names, seeds, ci_level=0.95):
    ...
    for agent_name in agent_names:
        ...
        for seed_idx, seed_df in enumerate(seed_data):
            # Align episodes (in case some seeds have different episode ranges)
            seed_episodes = seed_df['episode'].values
            episode_mask = np.isin(episodes, seed_episodes)
            
            # Map seed data to episode array
            episode_to_idx = {ep: idx for idx, ep in enumerate(episodes)}
            for _, row in seed_df.iterrows():
                ep = row['episode']
                if ep in episode_to_idx:
                    ep_idx = episode_to_idx[ep]
                    rewards_matrix[seed_idx, ep_idx] = row['total_reward']
                    steps_matrix[seed_idx, ep_idx] = row['steps']
                    success_matrix[seed_idx, ep_idx] = int(row['success'])
        
        # Calculate statistics
        std_err = np.std(rewards_matrix, axis=0, ddof=1) / np.sqrt(n_seeds)
        ...
        aggregated[agent_name] = {
            ...
            'rewards': {
                ...
                'ci_lower': np.mean(rewards_matrix, axis=0) - z_score * std_err,
                'ci_upper': np.mean(rewards_matrix, axis=0) + z_score * std_err
            },
            'steps': {
                'mean': np.mean(steps_matrix, axis=0),
                'std': np.std(steps_matrix, axis=0, ddof=1),
                'ci_lower': np.mean(steps_matrix, axis=0) - z_score * np.std(steps_matrix, axis=0, ddof=1) / np.sqrt(n_seeds),
                'ci_upper': np.mean(steps_matrix, axis=0) + z_score * np.std(steps_matrix, axis=0, ddof=1) / np.sqrt(n_seeds)
            },
            'success': {
                'mean': np.mean(success_matrix, axis=0) * 100,  # Convert to percentage
                'std': np.std(success_matrix, axis=0, ddof=1) * 100,
                'ci_lower': (np.mean(success_matrix, axis=0) - z_score * np.std(success_matrix, axis=0, ddof=1) / np.sqrt(n_seeds)) * 100,
                'ci_upper': (np.mean(success_matrix, axis=0) + z_score * np.std(success_matrix, axis=0, ddof=1) / np.sqrt(n_seeds)) * 100
            }
        }
...
def plot_aggregated_comparison(aggregated, agent_names, episodes_per_config=None, output_path=None, ci_level=0.95):
    ...
    ci_percent = int(ci_level * 100)
    fig.suptitle(f'Aggregated Comparison Across Multiple Seeds (with {ci_percent}% Confidence Intervals)', 
    ...
    for agent_name in agent_names:
        ...
        # For cumulative average, we need to propagate uncertainty
        # Using a simplified approach: approximate CI based on episode-wise CIs
        n = len(cumsum)
        cumavg_ci_lower = np.zeros(n)
        cumavg_ci_upper = np.zeros(n)
        
        for i in range(n):
            # Approximate: CI for cumulative average at episode i
            # This is a simplification - proper CI would require covariance
            mean_ci_lower = data['rewards']['ci_lower'][:i+1]
            mean_ci_upper = data['rewards']['ci_upper'][:i+1]
            cumavg_ci_lower[i] = np.mean(mean_ci_lower)
            cumavg_ci_upper[i] = np.mean(mean_ci_upper)
    ...
    for agent_name in agent_names:
        ...
        # Approximate CI for cumulative sum
...
def main():
    parser = argparse.ArgumentParser(description='Plot aggregated multi-seed comparison from separate seed files with confidence intervals')
    parser.add_argument('logs_dir', type=str, help='Path to directory containing seed CSV files')
    ...
    parser.add_argument('--ci_level', type=float, default=0.95,
                       help='Confidence interval level (default: 0.95 for 95% CI)')
    
    args = parser.parse_args()
    
    print(f"Loading seed files from: {args.logs_dir}")
    all_data, agent_names, seeds = load_seed_files(args.logs_dir)
    
    print(f"\n{'='*80}")
    print("Aggregating data across seeds...")
    print(f"{'='*80}")
    
    aggregated = aggregate_across_seeds(all_data, agent_names, seeds, args.ci_level)
    
    ci_percent = int(args.ci_level * 100)
    print(f"\nCreating plots with {ci_percent}% confidence intervals...")
    
    # Generate output path if not provided
    output_path = args.output
    if output_path is None:
        logs_path = Path(args.logs_dir)
        output_path = logs_path / f"eight_agents_aggregated_{len(seeds)}seeds.png"
    
    plot_aggregated_comparison(aggregated, agent_names, args.episodes_per_config, output_path, args.ci_level)
    
    print(f"\n✓ Aggregated comparison complete!")
    print(f"  Mean values aggregated across {len(seeds)} seeds")
    print(f"  Confidence intervals: {args.ci_level*100}%")
