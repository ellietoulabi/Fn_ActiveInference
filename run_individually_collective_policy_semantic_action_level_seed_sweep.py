    ...
    def run_one_episode(
        ...
        *,
        ...
        for step in range(1, max_steps_per_scenario + 1):
            ...
            a0_prim = (
                ric._env_primitive_from_policy_step(pol_0[0])
                if len(pol_0) > 0
                else int(model_init_agent.STAY)
            )
            a1_prim = (
                ric._env_primitive_from_policy_step(pol_1[0])
                if len(pol_1) > 0
                else int(model_init_agent.STAY)
            )
            ...
            if log_steps:
                ...
                print("    Primitive plan A0: {}".format(ric._fmt_policy(pol_0)), flush=True)
                print("    Primitive plan A1: {}".format(ric._fmt_policy(pol_1)), flush=True)
