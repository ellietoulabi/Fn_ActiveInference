"""
Compare Active Inference and Q-Learning agents in non-stationary environments.

Runs both agents on the same sequence of environment configurations
...
def main():
    print("="*80)
    print("COMPARING ACTIVE INFERENCE vs Q-LEARNING")
    print("Non-Stationary Environment - Button Positions Change")
    print("="*80)
    
    # Parameters
    NUM_EPISODES = 50
    EPISODES_PER_CONFIG = 10  # Change environment every 10 episodes
    MAX_STEPS = 50
    RANDOM_SEED = 42
    ...
    csv_filename = f"comparison_ep{NUM_EPISODES}_step{MAX_STEPS}_{timestamp}.csv"
    ...
    ql_agent = QLearningAgent(
        action_space_size=6,
        q_table_path=None,  # Don't save during comparison
        learning_rate=0.1,
    ...
    try:
        for episode in range(1, NUM_EPISODES + 1):
            ...
            # Run both agents on same environment
            print(f"\nEpisode {episode}:")
            
            # Reset environment seed for fairness
            ...
            print(f"  AIF: {'✅ WIN' if aif_result['success'] else '❌ FAIL'} "
                  f"- {aif_result['steps']} steps, reward: {aif_result['reward']:+.2f}")
            
            ql_result = run_ql_episode(env, ql_agent, episode, MAX_STEPS, verbose=False, csv_writer=csv_writer)
            ql_results.append(ql_result)
            print(f"  Q-L: {'✅ WIN' if ql_result['success'] else '❌ FAIL'} "
                  f"- {ql_result['steps']} steps, reward: {ql_result['reward']:+.2f}, "
                  f"ε={ql_result['epsilon']:.3f}")
    finally:
        csv_file.close()
    ...
    print(f"\n{'Metric':<25} {'Active Inference':<20} {'Q-Learning':<20}")
    print("-" * 65)
    print(f"{'Success Rate':<25} {aif_success_rate:>6.1f}%             {ql_success_rate:>6.1f}%")
    print(f"{'Total Wins':<25} {aif_successes:>6}/{len(aif_results):<13} {ql_successes:>6}/{len(ql_results)}")
    print(f"{'Average Reward':<25} {aif_avg_reward:>+7.2f}             {ql_avg_reward:>+7.2f}")
    print(f"{'Average Steps':<25} {aif_avg_steps:>7.1f}             {ql_avg_steps:>7.1f}")
    
    # Per-configuration analysis
    print(f"\n\nPer-Configuration Performance:")
    print(f"{'Config':<8} {'Episodes':<12} {'AIF Wins':<12} {'QL Wins':<12} {'AIF Reward':<15} {'QL Reward':<15}")
    print("-" * 85)
    ...
    for config_idx in range(num_configs):
        ...
        aif_avg_r = np.mean([r['reward'] for r in aif_config])
        ql_avg_r = np.mean([r['reward'] for r in ql_config])
        
        print(f"{config_idx+1:<8} {start+1:>3}-{end:<8} "
              f"{aif_wins}/{len(aif_config)} ({aif_rate:>5.1f}%)  "
              f"{ql_wins}/{len(ql_config)} ({ql_rate:>5.1f}%)  "
              f"{aif_avg_r:>+7.2f}         "
              f"{ql_avg_r:>+7.2f}")
    ...
    fig.suptitle('Active Inference vs Q-Learning: Non-Stationary Environment', fontsize=14, fontweight='bold')
    ...
    ax.plot(episodes, aif_success, 'o-', alpha=0.3, color='blue', label='AIF (raw)')
    ax.plot(episodes[window-1:], aif_ma, '-', linewidth=2, color='blue', label=f'AIF (MA-{window})')
    ax.plot(episodes, ql_success, 's-', alpha=0.3, color='orange', label='Q-L (raw)')
    ax.plot(episodes[window-1:], ql_ma, '-', linewidth=2, color='orange', label=f'Q-L (MA-{window})')
    ...
    ax.set_ylabel('Success (1=win, 0=fail)')
    ...
    ax.plot(episodes, aif_cumulative, '-', linewidth=2, color='blue', label='Active Inference')
    ax.plot(episodes, ql_cumulative, '-', linewidth=2, color='orange', label='Q-Learning')
    ...
    ax.plot(episodes, aif_steps, 'o-', color='blue', label='Active Inference')
    ax.plot(episodes, ql_steps, 's-', color='orange', label='Q-Learning')
    ...
    ax.set_title('Efficiency (Fewer Steps = Better)')
    ax.legend()
    ax.grid(True, alpha=0.3)
    ...
    width = 0.35
    ax.bar(x - width/2, aif_config_rates, width, label='Active Inference', color='blue', alpha=0.7)
    ax.bar(x + width/2, ql_config_rates, width, label='Q-Learning', color='orange', alpha=0.7)
    ...
    plot_path = output_dir / "comparison_nonstationary.png"
