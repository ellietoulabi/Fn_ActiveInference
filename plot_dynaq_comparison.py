"""
Plot comparison between Vanilla Dyna-Q and Dyna-Q with Multiple Recency Biases.

Shows side-by-side comparison of 5 agents' performance metrics:
- Vanilla Dyna-Q
...
def plot_comparison(all_agent_stats, episodes_per_config=None, output_path=None, log_file=None):
    ...
    # Default colors for known agent types
    default_colors = {
        'Vanilla': '#2E86AB',        # Blue
        ...
        'SingleStep': '#2E86AB',     # Blue
        'TrajShort': '#A23B72',      # Purple
        'TrajLong': '#E63946'        # Red
    }
    
    # Auto-generate colors for any unrecognized agents
    fallback_colors = ['#2E86AB', '#A23B72', '#F18F01', '#E63946', '#06A77D', '#000000', 
                      '#8B5A3C', '#4B0082', '#FFD700', '#32CD32']
    
    colors = {}
    linestyles = {}
    for i, agent_name in enumerate(agent_names):
        if agent_name in default_colors:
            colors[agent_name] = default_colors[agent_name]
            linestyles[agent_name] = '-'
        else:
            colors[agent_name] = fallback_colors[i % len(fallback_colors)]
            linestyles[agent_name] = ['-', '--', ':', '-.'][i % 4]
    
    # Auto-generate title based on agent names
    title = f'Agent Comparison: {", ".join(agent_names[:3])}{"..." if len(agent_names) > 3 else ""} ({len(agent_names)} agents)'
    ...
    add_config_boundaries(ax5, 0, max_ep)
    ...
    print(f"\n{'Metric':<25} ", end='')
    for agent_name in agent_names:
        print(f"{agent_name:<18}", end='')
    print()
    print(f"{'-' * (25 + 18 * len(agent_names))}")
    ...
    for agent_name in agent_names:
        ...
        print(f"{wins}/{len(stats)} ({rate:.1f}%){'':<5}", end='')
    print()
    ...
    for agent_name in agent_names:
        ...
        print(f"{best:.2f} (Ep {best_ep:.0f}){'':<6}", end='')
    print()
    ...
    for agent_name in agent_names:
        ...
        print(f"{min_steps:.0f} steps (Ep {min_ep:.0f}){'':<2}", end='')
    print()
...
def main():
    parser = argparse.ArgumentParser(description='Plot Dyna-Q comparison from log file (5 agents)')
